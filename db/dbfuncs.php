...
function registerPlayer($firstname, $lastname, $club, $phonenumber, $email, $password, $password_repeat, $squad1, $squad2, $squad3) {
...
//        $error["email"] = true;
    ...
    $stmt = $dbh->prepare("INSERT into Players (firstname, lastname, club, password, phonenumber, email) VALUES (:firstname, :lastname, :club, :password, :phonenumber, :email);");
    ...
    // get id of player
    $stmt = $dbh->prepare("SELECT id FROM Players WHERE firstname=:firstname AND lastname=:lastname AND club=:club AND password=:password AND phonenumber=:phonenumber AND email=:email;");
    $stmt->bindParam("firstname", $firstname);
    $stmt->bindParam("lastname", $lastname);
    $stmt->bindParam("club", $club);
    $stmt->bindParam("password", md5($password . $globSalt));
